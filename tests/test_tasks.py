...
def test_create_duplicate_task():
    ...
    token = login_response.json()["access_token"]
    first_response = client.post("/tasks", json= {"title": "Unique_test_003"}, headers={"Authorization":f"bearer {token}"}) #modificar "title" en cada uso debido a la existencia de la tarea
    second_response = client.post("/tasks", json= {"title": "Unique_test_003"}, headers={"Authorization":f"bearer {token}"}) #modificar "title" en cada uso debido a la existencia de la tarea
...
def test_delete_task():
    ...
    token = login_response.json()["access_token"]
    create_task = client.post("/tasks", json={"title":"Task_to_delete_003"}, headers={"Authorization":f"bearer {token}"}) #Modify Task number between requests
